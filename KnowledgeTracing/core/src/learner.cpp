...
Learner::Learner(const std::string& name, const std::vector<Exercise>& exs) {
    this->name = name;
    this->exercises_not_done = exs;
}

Learner::Learner(const Learner& o_learner) {
    this->name = o_learner.name;
    this->exercises_not_done = o_learner.exercises_not_done;


}


BKTModel& Learner::getBKT() {
    return this->BKT;
}
...
/* Modification that needs to be pushed by Quentin */
void Learner::setExercises(std::vector<Exercise> exoVec) {
    this->exercises_not_done = exoVec;
}

void Learner::setExercise(Exercise ex) {
    exercises_not_done.push_back(ex);
}

std::string Learner::getName() {
    return this->name;
}

void Learner::getActualSkill(const Skill& skill) {
    getMultipleActualSkills(std::vector<Skill>{skill});
}

void Learner::getMultipleActualSkills(const std::vector<Skill>& skills) {
...
void Learner::getAllActualSkills() {
...
void Learner::getVarsValues(const std::vector<std::string>& vars) {
    this->all_results.getLastResultsForVars(vars);
}


PGscore Learner::getPGscore(std::vector<plComputableObjectList>& new_results, Exercise& exo) {
    PGscore new_score;
...
double Learner::getPGmean(std::vector<plComputableObjectList>& new_results) {
    double somme = 0;
    size_t last_idx = 0;
...
double Learner::getPGmin(std::vector<plComputableObjectList>& future, Exercise& exo) {
    ...
    for (const auto& res: future) {
        for (auto var: res) {
            std::string skillName = Tools::getSkillNameFromResults(var.get_print_name(),
                                                                   "(", ")", "_");
            if (!isAvailable(skillName, exercises_not_done)) {
...
std::vector<int> Learner::getIndexes(const std::string& skill_name) {
...
std::vector<Exercise>& Learner::getExercisesLeft() {
    return this->exercises_not_done;
}

ResultStorage& Learner::getResultStorage() {
    return this->all_results;
}

void Learner::setBKT(BKTModel& new_BKT) {
    this->BKT = new_BKT;
...
bool Learner::isDisplayed(std::vector<std::string>& list, const std::string& skill_name) {
    return std::any_of(list.begin(), list.end(),
                        [skill_name](const std::string& elem) {return elem == skill_name;});
}

int Learner::triggerIsUsed(const plVariable& trigger, std::vector<plVariable>& res) {
...
bool Learner::isAvailable(const std::string& skill_name, std::vector<Exercise>& new_exercisesNotDone){
...
int Learner::exerciseVectorIndex(const Exercise& ex, std::vector<Exercise>& new_exercisesNotDone) {
...
void Learner::initDBN() {
    ...
    for (const auto& comp_obj: comp_obj_lst) {
        ...
        if ((comp_obj_name.find("Res") == std::string::npos) && (comp_obj_name.find("Trigger") == std::string::npos) &&
            ...
            //std::cout << "[learner.cpp l.262] SKILL: " << comp_obj_name << std::endl;
...
void Learner::doExercise(const Exercise& ex, const ExoEval& evaluation, const std::vector<VarAndEvidence>& add_evidences) {
    ...
    if (this->instant == 0) {
        ...
        for (size_t i = 0; i < ex.getSkillsUsed().size(); i++) {
            ...
            //std::cout << "[learner.cpp l.295] SKILL = " << ex.getSkill(i).name << std::endl;
        ...
        int pos = exerciseVectorIndex(ex, this->exercises_not_done);
        if (pos >= 0) {
            this->exercises_not_done.erase(exercises_not_done.begin() + pos);
        }
    ...
    // t > 0
        ...
        for (size_t i = 0; i < ex.getSkillsUsed().size(); i++) {
            ...
            //std::cout << "[learner.cpp l.357] SKILL = " << ("Skill_" + ex.getSkill(i).name) << std::endl;
...
Exercise& Learner::recommendExercise(AnswersPrediction decision, Metric pedagogy, const std::vector<VarAndEvidence>& add_evidences) {
    plVariablesConjunction variables_t = this->dbn_15.getVariables();
    ...
    if(this->instant == 0){
        ...
        for (auto& exo: this->exercises_not_done) {
            std::vector<plVariable> res;
            std::vector<plVariable> skills;
            plValues evidences_t0;
        ...
        for (auto& exo: this->exercises_not_done) {
            ...
            std::vector<plVariable> skills;
            plValues evidences;
            //std::cout << "skills_used.name() " << this->exercises_not_done[0].skills_used[0].name << std::endl;
    ...
    return this->exercises_not_done[index];
}

void Learner::exportResults(const std::string& export_type, const std::string& filename, const std::string& path) {
    this->all_results.outputToFile(export_type, filename, path);
}

void Learner::serialize(const std::string& filename) {
...
void Learner::deserialize(const std::string& filename) {
