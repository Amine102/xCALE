...
#ifndef KNOWLEDGE_TRACING_LEARNER_H
#define KNOWLEDGE_TRACING_LEARNER_H
...
/**
 * The prediction of the BKT for the answer to an exercise. It's either None (no evidence on how the learner will answer)
 * or Optimistic (the learner will always answer correctly).
 */
...
/**
 * The metric used to compute the score of each exercise. It's either PGMean (the average level of all skills of the learner);
 * or PGMin (the skill in which the learner is the weakest).
 */
enum class Metric {
    PG_min, PG_mean
};
...
class Learner {
public :
    Learner(); /*Default constructor needed for setExercise tgo work in iterations*/ 
    /**
     * Constructor for the Learner class.
     * @params name The name of the learner.
     * @params exs The exercises vector.
     */
    Learner(const std::string &name, const std::vector<Exercise> &exs);

    std::string getName();

    /**
     * Copy constructor for the Learner class
     * @param o_learner the learner to copy
     */
    Learner(const Learner& o_learner);

    /**
     * Getter for the BKT attribute in the Learner class.
     * @return The BKT.
     */
    BKTModel &getBKT();
    ...
    /**
     * Finds the skill in the results vector (class member) of the learner class and returns its indexes to display it
    ...
    /**
     * Sets the BKT for the learner, and adds all skills to the ResultStorage (for displaying purpose)
     ...
     * @params exercises_not_done Vector of all the exercises available.
     * @return Returns a boolean.
     */
     ...
     * @params exercises_not_done Vector of available exercises.
    ...
    /**
     * Updates the skills in the 1.5dbn linked to a Learner by giving them an exercise, answers to that exercise and
    ...
    /**
     * Recommends the next exercise available to do for a learner based on Metric and Answers prediction.
    ...
    /**
     * Exports the current results stored into the ResultStorage object of the student.
     */
    void exportResults(const std::string& export_type, const std::string& filename, const std::string& path = "");

    /**
     * Serializes the Learner's DBN, 1.5dbn and exercise/skills used to an XML file
     * @param filename The desired output filename
     */
    void serialize(const std::string& filename);

    /**
     * Loads a Learner (name, DBN, 1.5dbn and exercise/skills used) from an XML file
...
private:
    /** @brief Name of the learner */
    std::string name;

    /** @brief the BKT model associated to the learner */
    BKTModel BKT;

    /** @brief the 1.5 DBN created from the BKT's DBN */
    pm15dBN dbn_15;

    /** @brief current instant for the learner (-1 = DBN is not initialised) */
    int instant = -1;
    ...
    /** @brief Vector of exercises still not done by the learner */
    std::vector<Exercise> exercises_not_done;
};

#endif //KNOWLEDGE_TRACING_LEARNER_H