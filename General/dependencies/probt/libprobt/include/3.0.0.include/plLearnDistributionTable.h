...
class PL_DLL_API plLearnDistributionTable : public plLearnCndObject
{
  ...
  */
  ...
  /** Creates the distribution table corresponding to the learnt conditional
      ...
      If no default learning object is provided using \em push_default, then
      the returned distribution table does not contain a default function for the
      right values that have never been inserted. Consider calling \em
      push_default on the returned distribution table in order to customize it.
  ...
  */
  ...
  unsigned int get_nsamples(const plValues &right_value)const;
