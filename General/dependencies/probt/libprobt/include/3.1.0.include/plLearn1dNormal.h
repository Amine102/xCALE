...
public:
  ...
  typedef plVariable VarType;
  
  /** Constructor using a given variable.*/
  explicit plLearn1dNormal(const plVariable &vars) 
  ...
  plLearn1dNormal(const plVariable &vars, plFloat init_mean,
  ...
  */
  ...
  {
    total_weight_ = init_weight_ + s*(total_weight_ - init_weight_);
    s_ = init_weight_*init_mean_ + s*(s_ - init_weight_*init_mean_);
    const plFloat tt = init_weight_*(init_mean_*init_mean_ + init_std_dev_*init_std_dev_);
    s2_ = tt + s*(s2_ - tt);
  }
