...
public:
  ...
  /** Constructs a vector having \em r elements initialized to \em val.
  */
  ...
  */
  ...
  /** Return the ith element of the vector. It is assumed that 0 <= i <
      size(). */
  inline plFloat operator[](size_t i) const {
    return v_[i];
  }

  /** Return a reference to the ith element of the vector.  It is assumed that
      0 <= i < size(). */
  inline plFloat& operator[](size_t i) {
    return v_[i];
  }
