...
class PL_DLL_API plFloatMatrix :public plObject
{
 ...
 template<typename T>
   void set_values( size_t r, size_t, T float_vector);

  template<typename T>
    void set_values(const std::vector <std::vector <T> > &stl_matrix);
  
  template <typename T>
    void set_square_from_vector(const std::vector <T> &float_vector);

  template <typename T>
    void set_from_vector(size_t r, size_t c, const std::vector <T> &float_vector);
  ...
  */
  ...
  /** Constructs a matrix having \em r x \em c  elements  and fills it using the
      \em float_vector C array of floating-point values so that
      \em m[i][j] = float_vector[i*r+j] for i = 0..r-1, j = 0..c-1.
  */
  ...
  /** Constructs a matrix having \em r x \em c  elements  and fills it using the
      \em float_vector SL vector of floating-point values so that
      \em m[i][j] = float_vector[i*r+j] for i = 0..r-1, j = 0..c-1.
  */
  ...
  plFloatMatrix(size_t r, size_t c, const std::vector <long double> &float_vector);
  ...
  /** Constructs a diagonal nxn square matrix having \em diag_value as diagonal value.
      Previous content is lost. */
  void make_diagonal(size_t n, plFloat diag_value=PL_ONE);
  ...
  */
  ...
  /** Inverses the matrix if inversible and returns the pseudoinverse if the matrix is not
      ...
      \returns  The resulting inverse or pseudoinverse matrix 
      ...
      multiple solutions (under-determined systems: nrows < ncolmns).
  */
  plFloatMatrix inverse_using_greville(plFloat threshold) const
  {
    plFloatMatrix inv;
    inverse_using_greville(inv, threshold);
    return inv;
  }
  ...
  */
  plFloatMatrix inverse_using_eigen_decomposition() const
  {
    plFloatMatrix inv;
    inverse_using_eigen_decomposition(inv);
    return inv;
  }
  ...
  */    
  ...
  static plFloatMatrix diagonal(size_t n, plFloat val=PL_ONE)
  {
    plFloatMatrix ret;
    ret.make_diagonal(n, val);
    return ret;
  }

#ifdef PL_HAVE_CPP11
  /** Move constructor */
  plFloatMatrix(plFloatMatrix&& tmp_other);
  
  /** Move assignment */
  plFloatMatrix& operator=(plFloatMatrix&& tmp_other);
#endif
