...
public:
  ...
  */
  ...
  /** Return False if the determinante of the covariance matrix is > 0 
      (standard deviation > 0 in one-dimensional case) 
  */
  bool variance_is_ok() const;

  /** Set/change the minimal allowed standard deviation (default is 1e-10)
      
      Standard deviation values that are less than this value are replaced by this value 
      
      \sa get_min_standard_deviation()
  */  
  static void set_min_standard_deviation(plFloat val);
  
  
  /** Get the minimal allowed standard deviation
      
      \sa set_min_standard_deviation()
  */    
  static plFloat get_min_standard_deviation();
 ...
 private:
  static plFloat min_standard_deviation_;
};
