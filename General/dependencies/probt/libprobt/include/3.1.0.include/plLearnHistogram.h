...
public:
  ...
  plLearnHistogram(const plVariablesConjunction &vars,
                   const std::vector<unsigned int> &init_freq, plFloat init_weight=PL_ONE);

  plLearnHistogram(const plVariablesConjunction &vars,
                   const std::vector<plFloat> &init_freq, plFloat init_weight=PL_ONE);
  ...
  */
  ...
  void get_frequency(T *table)const
  {
    ...
    for(size_t i = 0; i < freq_.size(); ++i) table[i] = freq_[i];
  }
  ...
  virtual void rescale_total_weight(plFloat s)
  {
    ...
    for(size_t i = 0; i < freq_.size(); ++i) {
      freq_[i] = init_freq_[i] + (freq_[i]-init_freq_[i])*s;
    }
  }
