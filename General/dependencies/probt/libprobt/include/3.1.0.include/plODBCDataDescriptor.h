...
public:
    ...
    if (ODBC_->next () == 0) { //no error in accessing datasource
      for(size_t i = 0; i < current_row_.size(); ++i){
        ...
        if(current_row_def_[i]) {
          if(is_label_[i]) {
            current_row_[i] = ODBC_->get_column_value<std::string> (i);
          }
          else {
            current_row_[i] = ODBC_->get_column_value<plFloat> (i);
          }
        }
      }
  ...
  */
  inline void set_variables(const plVariablesConjunction &variables)
  {
    ...
    is_label_.resize(get_num_fields());
    for(unsigned int i = 0; i < get_num_fields(); ++i) {
      is_label_[i] = (variables_[i].get_var_type() == PL_LABEL);
    } 
  ...
  plValues create_row_values() const
  {
    return (!variables_.is_empty() ?
            plValues(variables_) :
            plValues(plVariableCollection("X", plRealType(-std::numeric_limits<plFloat>::max(), std::numeric_limits<plFloat>::max()),
                                          1, num_fields_)));
  }
...
inline plDatabaseInitError plODBCDataDescriptor::init()
{
  ...
  current_row_=  create_row_values();
