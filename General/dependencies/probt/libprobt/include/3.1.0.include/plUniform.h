...
public:
  ...
  explicit plUniform(const plVariable &variable);
  ...
  */
  ...
  /** Create a multi-dimensional uniform on a set of variables 
      It's equivalent to plDistribution( plUniform(variables[0])*plUniform(variables[1])*...*plUniform(variables[variables.size()-1]) )
  */
  static plDistribution create_uniform(const plVariablesConjunction &variables);
