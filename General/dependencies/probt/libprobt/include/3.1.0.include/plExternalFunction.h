...
class PL_DLL_API plExternalFunction : public plUserFunction
{
  ...
  */
  ...
  /** Logical or. The input values are interpreted as booleans. The output
      value is a 0/1 value representing the logical OR between all the
      inputs. */
  static void logical_or(plValues &or_result, const plValues &vals);

  /** Logical and. The input values are interpreted as booleans. The output
      value is a 0/1 value representing the logical AND (conjunction) between
      all the inputs. */
  static void logical_and(plValues &and_result, const plValues &vals);
};
...
/** Creates a plExternalFunction from an object with the appropriate f() method.
    ...
    @param[in] func_name Optional function name for pretty-printing.
