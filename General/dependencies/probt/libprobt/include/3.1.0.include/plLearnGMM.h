...
class PL_DLL_API plLearnGMM :public plLearnMixtureModel
{
  ...
  */
  ...
  /** Constructor 
      ...
      \param[in] independent_dimensions If 'true', ObsVars are assumed to be independent (i.e., diagnonal covariance matrix) 
  */
 ...
 protected:
  ...
  /** Get an initialization Normal distribution (cluster) P(X | C=c).
      ...
      @code
      plDistribution plLearnGMM::get_initialization_cluster(const plVariablesConjunction &X, size_t nc, size_t trial)
      {
        if(X.dim() == 1) {
          return plNormal(plVariable(X), 
  ...
  */
  ...
  explicit plLearnGMM(const plVariablesConjunction &ObsVars);
