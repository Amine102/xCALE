...
class PL_DLL_API plEMLearner  : public plLearner
{
  ...
  */
  ...
  void add_component(const plLearnObject &learn_object);
  
  /** Add a new component by providing the corresponding learning object and the initial distribution */
  void add_component(const plLearnObject &learn_object, const plComputableObject &init_distribution);
  
  /** Set the initial distributions */
  void set_initial_distributions(const plComputableObjectList &init_distributions);
  ...
  */
  ...
  /** Run the EM algorithm until convergence:\n 
      ...
      @param[in,out] data_descriptor the data set on which the EM
      algorithm will iterate.  It can contain arbitrary missing data.
      
      @param[in] convergence_loglikelihood_threshold convergence threshold of
      relative log-likelihood change between two successive EM iterations
      (smaller value means more iterations). The default value is set to 0.0001
  ...
  */
  unsigned int run(plDataDescriptor &data_descriptor,
                   plFloat convergence_loglikelihood_threshold=0.0001,
                   unsigned int max_it = std::numeric_limits<unsigned int>::max());

  /** Run the EM algorithm for a given number of iterations and return the last iteration info.
      
      @param[in,out] data_descriptor the data set on which the EM algorithm will
      iterate.  It can contain arbitrary missing data.
      @param[in] nit the number of iterations for which the EM algorithm will
      be run.
  ...
  */
  ...
  /** If 'same' = true, the learning data is assumed containing missing values
      for the same variables in all data rows (i.e., unobserved/latent variables exclusively).
  ...
  */
  void set_approximate_compilation_time(plFloat compilation_time_in_seconds)
  { 
 ...
 private:
  ...
  std::vector <plVariable> descriptor_variables_;
  plVariablesConjunction current_def_vars_;
  plVariablesConjunction current_undef_vars_;
  std::vector <unsigned int> descriptor_current_def_variables_;
  std::vector <unsigned int> descriptor_old_def_variables_;
  std::vector <unsigned int> current_undef_variables_; 
  std::vector <unsigned int> static_undef_variables_; 
  plLearnDistributions *learn_distrib_vector_;
  ...
  plVariablesConjunction static_undef_vars_;
  ...
  plFloat approx_compilation_time_;
