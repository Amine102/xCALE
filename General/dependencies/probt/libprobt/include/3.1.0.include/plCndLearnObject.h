...
class PL_DLL_API plCndLearnObject : public plLearnDistributionTable
{
 public:
  ...
  /** Constructor to learn the conditional distribution P(left_vars |
      ...
      returned distribution when get_distribution_table() (or 
      get_computable_object()) is called will contain a default function obtained
      by calling get_distribution() on \em init_object.
  */
  ...
  */
  ...
  template <typename TT>
    friend plCndDistribution get_cnd_distribution_impl(const plCndLearnObject<TT>& obj, const std::string &Tname);
