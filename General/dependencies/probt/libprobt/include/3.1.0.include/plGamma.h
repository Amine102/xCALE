...
public:
  ...
  /** Constructs a Gamma distribution on the domain of @em variable.
      ...
      @param[in] variable Variable over which the distribution is built.
  ...
  */
  plGamma(const plVariable& variable, plFloat alpha, plFloat theta, plFloat mu = PL_ZERO );
...
/** Log-Gamma function */
inline plFloat plLgamma(plFloat x)
{
  return std::lgamma(x);
}
