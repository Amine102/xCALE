...
class PL_DLL_API plJointDistribution : public plDistribution
{
  ...
  **/
  void draw_graph_dot(std::ostream& out, 
                      const std::string &dot_graph_label="",
  ...
  **/
  ...
  {
    THROW_PL_ERROR(89, "plJointDistribution.python_draw_graph() is only available for Python");
  }
  ...
  */
  ...
  /** Creates a conditional probability expression  P(searched_variables | known_variables) using Monte Carlo
      ...
      (i.e., computation of the sums/integrals). When negative, the atual number will be 
      n = -mc_nsamples x free_vars_dim in which free_vars_dim is the number of variables on which we will marginalize
      \param[in] do_not_build_normalization_expression Set it to
      @em true, if you don't need to compute the normalization any more.  You
      should use the default value (@em false) unless you know what you are
      doing.
  ...
  */
  ...
  plVariablesConjunction get_markov_blanket(const plVariable&s) const;
  ...
  */
  plVariablesConjunction get_soft_evidence_variables() const;
  ...
  */
  ...
  {
    return get_computable_object_list().ancestral_order_variables();
  }
