...
{ 
  ...
  */
  ...
  /** Construct a plDistributionTable with an array of plProbValues implicitly
      defining a set of plProbTables. Use this constructor only when you need
      to change the conditional distribution later. Consider using the more
      efficient constructor plCndDistribution(const plVariablesConjunction&,
      const plVariablesConjunction&, const plProbValue *, bool) if you do not
      need to change it.
  ...
  */
    ...
    : plCndDistribution(left, right, PLDistributionTable)
  {
    ...
    const size_t the_size = table.size();
  ...
  */
    ...
    : plCndDistribution(left, right, PLDistributionTable)
  {
    ...
    plValues v_left(left);
    plValues v_right(right);
    std::vector<plProbValue> column_values(size_t(left.cardinality()));
    do {
      v_left.reset();
      size_t i = 0;
      do {
        column_values[i++] = (obj->*method)(v_left, v_right);
      } while (v_left.next());
      push(v_right, plProbTable(left, column_values, already_normalized));
    } while (v_right.next());
  }
