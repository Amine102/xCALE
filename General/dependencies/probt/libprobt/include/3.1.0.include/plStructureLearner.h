...
{
 public:
  /// An edge sequence as a vector of vertex plVariable pairs.
  typedef std::vector< std::pair<plVariable, plVariable> > variable_edge_list_t;
  ...
  /// Construct a structure learner that will operate on the given variables
  /// and the given initial structure.
  explicit plStructureLearner(const plVariablesConjunction& variables, 
                              const variable_edge_list_t& init_edges=variable_edge_list_t());
  /// Copy constructor
  ...
  bool DMST(const plEdgeScore& score,
            std::vector<plVariable>& vertex_order,
            const plVariable &root_node,
            const variable_edge_list_t& included_edges = variable_edge_list_t(),
            const variable_edge_list_t& excluded_edges = variable_edge_list_t());
  ...
  void K2(const plNodeScore& score,
          const std::vector<plVariable>& vertex_order,
          const variable_edge_list_t& included_edges = variable_edge_list_t(),
          const variable_edge_list_t& excluded_edges = variable_edge_list_t(),
  ...
  plJointDistribution get_joint_distribution(plDataDescriptor& data_desc) const;
  ...
  std::vector< std::pair<plVariable, plVariable> > get_edges() const;
 ...
 private:
  ...
  std::vector<plVariable> indices_to_variables(const std::vector<unsigned int> &indices) const;
  
  std::vector<unsigned int> variables_to_indices(const std::vector<plVariable> &nodes) const;
  
  unsigned int variable_to_index(const plVariable &s) const;

  const plVariable& index_to_variable(unsigned int i) const;

  edge_list_t variable_pairs_to_indice_pairs(const variable_edge_list_t& edges) const;
