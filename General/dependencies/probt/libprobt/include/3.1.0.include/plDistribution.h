...
class PL_DLL_API plDistribution : public plComputableObject
{
  ...
  */
  void n_compile(plDistribution& result,
    ...
    /**
     Does a n_compile(), and returns the compiled distribution. See n_compile()
     above for details.
     ...
     instead.
  */
  plDistribution n_compile(unsigned int n,
                           plGeneratorType generator_type = PL_CHOOSE_GENERATOR_FOR_ME,
                           plCompiledDistributionType compiled_distrib_type 
                           = PL_CHOOSE_COMP_TYPE_FOR_ME) const;
  ...
  */
  ...
  /** Like n_compile() with the possibility to incrementally compile the
      ...
      update it. It also assumes that this result distribution is empty for the
  ...
  */
  ...
  /** Set a soft evidence on the expression. 
      ATTENTION: after this call, the values provided by compute() functions is no longer normalized. Compile you distribution to get normalized values
  */
  ...
  */
  ...
  void remove_soft_evidence(const plVariable &variable);
